...
@Directive({
  selector: '[appAppDrag1]'
})
export class AppDrag1Directive implements OnInit {
    ...
    @HostListener('touchmove', ['$event']) touchmove(event) {
       event.preventDefault();
        ...
        if (this.isDown) {
            ...
            let xP = x;
            let yP = y;
            const widthC =  Math.abs(parseFloat(this.nodeWidth) - parseFloat(this.parentNodeWidth));
            const heightC =  Math.abs(parseFloat(this.nodeHeight) - parseFloat(this.parentNodeHeight));
            if ( x <=  -widthC ) {
                xP =  -widthC ;
            } else if (x >=  0) {
                xP =  0;
